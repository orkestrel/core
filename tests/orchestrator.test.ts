...
describe('Orchestrator suite', () => {
	...
	test('tracer emits layers and per-phase outcomes', async () => {
		...
		const phases: Array<{ phase: 'start' | 'stop' | 'destroy'; layer: number; outcomes: Array<{ token: string; ok: boolean }> }> = [];
		const app = new OrchestratorAdapter(new ContainerAdapter({ logger }), {
			logger,
			tracer: {
				onLayers: (payload: { layers: string[][] }) => { layersSeen.push(payload.layers); },
				onPhase: (payload: { phase: 'start' | 'stop' | 'destroy'; layer: number; outcomes: Array<{ token: string; ok: boolean }> }) => {
	...
	test('tracer start outcomes include failures (first)', async () => {
		...
		const phases: Array<{ phase: 'start' | 'stop' | 'destroy'; layer: number; outcomes: Array<{ token: string; ok: boolean; timedOut?: boolean }> }> = [];
