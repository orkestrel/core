...
export class OrchestratorAdapter {
		...
		for (let i = 0; i < layers.length; i++) {
			...
			if (failures.length > 0) {
				...
				for (const batch of this.#groupByLayerOrder(toStop.map(x => x.token))) {
					const stopJobs: Task<{ outcome: Outcome; error?: LifecycleErrorDetail }>[] = []
	...
	async #runLayerWithTracing<J>(phase: LifecyclePhase, layerIdxForward: number, jobs: readonly Task<J>[], toOutcome: (j: J) => Outcome | undefined): Promise<readonly J[]> {
