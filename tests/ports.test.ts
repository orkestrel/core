...
import { extendPorts, createPortToken, createPortTokens, OrchestratorAdapter, Adapter, NoopLogger } from '@orkestrel/core';
...
describe('Ports suite', () => {
	test('createPortTokens creates a base token set; extendPorts merges', async () => {
		...
		const orch = new OrchestratorAdapter({ logger });
